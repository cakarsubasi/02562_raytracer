...
function build_bsp_tree(drawingInfo, device, buffers)
{
  ...
  device.queue.writeBuffer(buffers.positions, 0, g_drawingInfo.vertices);

  buffers.normals = device.createBuffer({
    size: g_drawingInfo.normals.byteLength,
    usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.STORAGE
  });
  device.queue.writeBuffer(buffers.normals, 0, g_drawingInfo.normals);

  buffers.colors = device.createBuffer({
    size: g_drawingInfo.colors.byteLength,
    usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.STORAGE
  });
  device.queue.writeBuffer(buffers.colors, 0, g_drawingInfo.colors);

  buffers.indices = device.createBuffer({
    size: g_drawingInfo.indices.byteLength, 
    usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.STORAGE
  });
  device.queue.writeBuffer(buffers.indices, 0, g_drawingInfo.indices);
...
function intersect_triangle(r, hit, idx)
{
  let face_x = g_drawingInfo.indices[idx*4]*4;
  let face_y = g_drawingInfo.indices[idx*4 + 1]*4;
  let face_z = g_drawingInfo.indices[idx*4 + 2]*4;
  let v0 = vec3(g_drawingInfo.vertices[face_x], g_drawingInfo.vertices[face_x + 1], g_drawingInfo.vertices[face_x + 2]);
  let v1 = vec3(g_drawingInfo.vertices[face_y], g_drawingInfo.vertices[face_y + 1], g_drawingInfo.vertices[face_y + 2]);
  let v2 = vec3(g_drawingInfo.vertices[face_z], g_drawingInfo.vertices[face_z + 1], g_drawingInfo.vertices[face_z + 2]);
